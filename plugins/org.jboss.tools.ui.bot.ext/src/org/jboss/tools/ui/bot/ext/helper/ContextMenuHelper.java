...
public class ContextMenuHelper {
	 ...
	 *
	 * @param bot
	 *            bot containing context menu.
	 * @param text
	 *            the text on the context menu.
	 * @param hideAfterwards
	 *            hide menu when method is finished.
	 * @throws WidgetNotFoundException
	 *             if the widget is not found.
	 */
	public static MenuItem getContextMenu(final AbstractSWTBot<?> bot, final String text,
			final boolean hideAfterwards) {
		final Menu menu = Display.syncExec(new ResultRunnable<Menu>() {
			public Menu run() {
	 ...
	 *
	 * @param menu
	 *            top parent menu containing menu item.
	 * @param text
	 *            the text on the context menu.
	 * @param hideAfterwards
	 *            hide menu when method is finished.
	 ...
	 *             if the widget is not found.
	 */
	public static MenuItem getContextMenu(final Menu menu, final String text, final boolean hideAfterwards) {
		final List<String> foundMenuItems = new Vector<String>();
		final MenuItem menuItem = Display.syncExec(new ResultRunnable<MenuItem>() {
			public MenuItem run() {
		...
		if (menuItem == null) {
			throw new WidgetNotFoundException(
					"Could not find menu: '" + text + "', found items :" + Arrays.toString(foundMenuItems.toArray()));
		} else {
			if (hideAfterwards) {
				// hide
	 ...
	 *            list of menuItems found (useful for debugging), items found in
	 ...
	 * 
	 * @param menu
	 *            bottom menu to start hiding from
	...
	/**
	 * Executes proper steps to be able call getContextMenu on first Tree Item
	 * within tree
	 * 
	 * @param tree
	 */
	...
	/**
	 * Executes proper steps to be able call getContextMenu on Tree Item within
	 * tree on specified position
	 * 
	 * @param tree
	 * @param index
	 *            - zero based index of Tree Item to be selected
	 */
	 ...
	 * 
	 * @param topMenu
	 *            first menu in submenu path to requested menu item
	 * @param text
	 *            the text on the context menu.
	 * @throws WidgetNotFoundException
	 *             if the widget is not found.
	 ...
	 * 
	 * @param control
	 *            Control containing Menu
	 * @param text
	 *            the text on the context menu.
	 * @throws WidgetNotFoundException
	 *             if the widget is not found.
	 */
	public static void clickContextMenu(final Control control, final String... texts) {

		final Menu topMenu = Display.syncExec(new ResultRunnable<Menu>() {
			public Menu run() {
	...
	public static void clickContextMenu(final AbstractSWTBot<?> bot, final String... texts) {
		...
		final Control parentControl = Display.syncExec(new ResultRunnable<Control>() {
			public Control run() {
	 ...
	 * 
	 * @param editor
	 *            editor containing requested Context Menu
	 * @param index
	 *            index of StyledText widget containing requested Context Menu
	 *            within editor
	 * @param text
	 *            the text on the context menu.
	 * @throws WidgetNotFoundException
	 *             if the widget is not found.
	 */
	public static void clickContextMenu(final SWTBotEditor editor, int index, final String... texts) {
		editor.setFocus();
	 ...
	 *             if the widget is not found.
	 */
	public static void clickContextMenu(final SWTBotEditor editor, final String... texts) {
	...
	private static void click(final MenuItem menuItem) {
		...
		final boolean isEnabled = Display.syncExec(new ResultRunnable<Boolean>() {
			public Boolean run() {
			...
			public Menu run() {
				...
				try {
					Menu[] menus = ReflectionsHelper.getPrivateFieldValue(
							SWTJBTExt.isRunningOnMacOs() ? org.eclipse.swt.widgets.Display.class : Decorations.class,
	...
	public static void treeRightClick(final Tree tree, final TreeItem treeItem) {
		Rectangle cellBounds = Display.syncExec(new ResultRunnable<Rectangle>() {
	 ...
	 * @since 1.2
	 */
	protected static Event createMouseEvent(int x, int y, int button, int stateMask, int count, Tree tree,
			TreeItem treeItem) {
		Event event = new Event();
		event.time = (int) System.currentTimeMillis();
		event.widget = tree;
	...
	public static String[] getMenuItemLabels(final Menu menu) {
		...
		result = Display.syncExec(new ResultRunnable<String[]>() {
			public String[] run() {
				MenuItem[] items = menu.getItems();
	...
	public static Menu showMenuOfMenuItem(final MenuItem menuItem) {
		...
		result = Display.syncExec(new ResultRunnable<Menu>() {
			public Menu run() {
	...
	/**
	 * Trim special characters from menu item label used before comparing menu
	 * item label
	 * 
	 * @param menuItemLabel
	 * @return
