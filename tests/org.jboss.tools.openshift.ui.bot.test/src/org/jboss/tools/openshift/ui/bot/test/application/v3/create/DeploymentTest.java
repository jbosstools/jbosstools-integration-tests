	...
	public void testDeploymentOfApplicationCreatedFromTemplate() {
		...
		try {
			new WaitUntil(new ResourceExists(DatastoreOS3.SERVER, DatastoreOS3.USERNAME,
					...
					ResourceState.RUNNING), TimePeriod.VERY_LONG, true, TimePeriod.getCustom(7));
		} catch (WaitTimeoutExpiredException ex) {
			fail("There should be a pod running an application build, but there is not.");
		}
		
		try {
			new WaitUntil(new ResourceExists(DatastoreOS3.SERVER, DatastoreOS3.USERNAME,
					DatastoreOS3.PROJECT1_DISPLAYED_NAME, Resource.POD, "eap-app-1-build",
					ResourceState.SUCCEEDED), TimePeriod.getCustom(600), true, TimePeriod.getCustom(7));
		} catch (WaitTimeoutExpiredException ex) {
		...
		try {
			new WaitUntil(new AmountOfResourcesExists(DatastoreOS3.SERVER, DatastoreOS3.USERNAME, 
					DatastoreOS3.PROJECT1_DISPLAYED_NAME, Resource.POD, 2), TimePeriod.getCustom(150),
		...
		try {
			new WaitUntil(new ResourceExists(DatastoreOS3.SERVER, DatastoreOS3.USERNAME, 
					DatastoreOS3.PROJECT1_DISPLAYED_NAME, Resource.BUILD, "eap-app-1", ResourceState.COMPLETE),
		...
		explorer.getOpenShift3Connection(DatastoreOS3.USERNAME, DatastoreOS3.PASSWORD).getProject(
				DatastoreOS3.PROJECT1_DISPLAYED_NAME).getOpenShiftResources(Resource.ROUTE).get(0).select();
