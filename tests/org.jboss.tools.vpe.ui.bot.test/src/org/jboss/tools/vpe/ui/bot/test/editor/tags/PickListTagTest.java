/*******************************************************************************
 * Copyright (c) 2007-20161 Exadel, Inc. and Red Hat, Inc.
	...
	protected void initTestPage() {
		...
		initTestPage(TestPageType.XHTML,
				"<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">\n"
						...
						+ "  <head>\n" + "  </head>\n" + "  <body>\n" + "    <f:view>\n"
						+ "      <rich:pickList value=\"\">\n" + sbOptions.toString() + "      </rich:pickList>\n"
						+ "    </f:view>\n" + "  </body>\n" + "</html>");
	}

	@Override
	protected void verifyTag() {
		assertVisualEditorContainsNodeWithValue(getVisualEditor(), "⇒ Copy all", getTestPageFileName());
		assertVisualEditorContainsNodeWithValue(getVisualEditor(), "→ Copy", getTestPageFileName());
		assertVisualEditorContainsNodeWithValue(getVisualEditor(), "⇐ Remove All", getTestPageFileName());
		assertVisualEditorContainsNodeWithValue(getVisualEditor(), "← Remove", getTestPageFileName());
		assertVisualEditorContains(getVisualEditor(), "TABLE", new String[] { "class" },
				new String[] { "rich-list-picklist" }, getTestPageFileName());
		assertVisualEditorContains(getVisualEditor(), "TABLE", new String[] { "class" },
				new String[] { "rich-picklist-body" }, getTestPageFileName());
		assertVisualEditorContains(getVisualEditor(), "TABLE", new String[] { "class" },
				new String[] { "rich-picklist-internal-tab" }, getTestPageFileName());
		assertVisualEditorContains(getVisualEditor(), "TBODY", null, null, getTestPageFileName());
		int index = 0;
		for (String option : options) {
			assertVisualEditorContainsNodeWithValue(getVisualEditor(), option, getTestPageFileName());
			...
			sbTitle.append(index);
			assertVisualEditorContains(getVisualEditor(), "OPTION", new String[] { "title" },
					new String[] { sbTitle.toString() }, getTestPageFileName());
			index++;
		}
		// check tag selection
		...
		AbstractWait.sleep(TimePeriod.getCustom(3));
		String currentLineText = getSourceEditor().getTextAtLine(getSourceEditor().getCursorPosition().x);
		...
		assertTrue("Current Line text in Source Pane has to be '" + sbOption.toString() + "'" + "\nbut it is '"
				+ currentLineText + "'", currentLineText.trim().equals(sbOption.toString()));
		// select second item in Source Editor
		getSourceEditor().activate();
		getSourceEditor().setCursorPosition(getSourceEditor().getCursorPosition().x + 1,
				getSourceEditor().getCursorPosition().y);
		AbstractWait.sleep(TimePeriod.getCustom(3));
		...
		assertVisualEditorContains(getVisualEditor(), "OPTION", new String[] { "title" },
				new String[] { sbOptionTitle.toString() }, getTestPageFileName());
	}
