...
/**
 * Helper to work with web browser used by JBoss Tools contained in JSP
 * MultiPage Editor
 * 
...
public class SWTBotWebBrowser {
	...
	public SWTBotWebBrowser(String title) {
		JSPMultiPageEditor multiPageEditor = new SWTJBTBot().multiPageEditorByTitle(title).getJspMultiPageEditor();
	...
	public static void displayNsIDOMNode(nsIDOMNode node, int depth) {
		...
		if (node.getNodeType() == Node.ELEMENT_NODE) {
			...
			for (int i = 0; i < modelAttributes.getLength(); i++) {
				...
				System.out.print(
						modelAttr.getNodeName() + "=\"" + modelAttr.getNodeValue().replaceAll("\n", " ") + "\" ");
			}
		}
		System.out.println(">");
		...
		for (int i = 0; i < children.getLength(); i++) {
			...
			if ((child.getNodeType() == Node.TEXT_NODE)
					&& ((child.getNodeValue() == null) || (child.getNodeValue().trim().length() == 0)))
				continue;
	...
	public void selectDomNode(final nsIDOMNode node, final int offset) {
		...
		Display.syncExec(new Runnable() {
			public void run() {
				getMozillaEventAdapter().notifySelectionChanged(getNsIDOMDocument(), selection,
	 ...
	 * @param tagName
	 * @param order
	 *            - index of tagName tag in DOM model from all tagName nodes
	 *            contained in model
	 * @return
	 */
	...
	public Browser getBrowser() {
		return (mozillaEditor != null) ? mozillaEditor.getXulRunnerEditor().getBrowser() : vpvEditor.getBrowser();
	}
	 ...
	 * @param toolLabel
	 */
	public static void activatePaletteTool(final String toolLabel) {
		final PaletteViewer paletteViewer = SWTBotWebBrowser.getPaletteViewer();
	...
	/**
	 * Returns true if node or it's child has node with specified name and
	 ...
	 * @return
	 */
	public boolean containsNodeWithNameAndAttributes(nsIDOMNode node, String searchName, String[] attributeNames,
		...
		if (node == null) {
			...
			int numNodes = Display.syncExec(new ResultRunnable() {
				@Override
				public Integer run() {
			...
			while (!result && nodeIndex < numNodes) {
				...
				if (attributeNames != null && attributeNames.length > 0) {
					...
					while (attrIndex < attributeNames.length && attributesMatches) {
						final String commandToGetAttributeValue = "return document.getElementsByTagName('" + searchName
								+ "')[" + nodeIndex + "].getAttribute('" + attributeNames[attrIndex] + "')";
						String attrValue = Display.syncExec(new ResultRunnable<String>() {
							@Override
							public String run() {
						...
						if (attributeNames[attrIndex].equalsIgnoreCase("style")) {
							...
							if (attributeValues[attrIndex].endsWith(";")) {
								attributeValues[attrIndex] = attributeValues[attrIndex].substring(0,
										attributeValues[attrIndex].length() - 1);
							}
						}
	...
	public boolean containsNodeWithValue(nsIDOMNode node, String searchText) {
		...
		if (node != null) {
			...
			// Use JavaScript
			final Browser swtBrowser = vpvEditor.getBrowser();
			String domTextContent = Display.syncExec(new ResultRunnable<String>() {
				@Override
				public String run() {
	 ...
	 * @return
	 */
	private static PaletteViewer getPaletteViewer() {
		new JBTPaletteView().open();

		IViewReference ref = Display.syncExec(new ResultRunnable<IViewReference>() {
			public IViewReference run() {
				IViewReference ref = null;
				IViewReference[] viewReferences = null;
				viewReferences = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage()
						.getViewReferences();
		...
		try {
			PaletteCreator pc = ReflectionsHelper.getPrivateFieldValue(PalettePageImpl.class, "paletteCreator", ppi,
					PaletteCreator.class);
			PaletteAdapter pa = ReflectionsHelper.getPrivateFieldValue(PaletteCreator.class, "paletteAdapter", pc,
					PaletteAdapter.class);
			PaletteViewer paletteViewer = ReflectionsHelper.getPrivateFieldValue(PaletteAdapter.class, "viewer", pa,
					PaletteViewer.class);
	...
	/**
	 * Returns Palette Entry from Palette Viewer paletteViewer with entryLabel
	 * label
	 * 
	 * @param paletteViewer
	 * @param entryLabel
	...
	private static PaletteEntry getPaletteEntry(PaletteViewer paletteViewer, String entryLabel) {
		...
		if (paletteEntries != null && paletteEntries.size() > 0) {
			...
			throw new WidgetNotFoundException("Unable to find JBoss Tools Palette Entry with label: " + entryLabel);
		}
	 ...
	 * @return
	 */
	public static boolean paletteContainsPaletteEntry(String paletteEntryLabel) {
		boolean result = false;
		PaletteViewer paletteViewer = SWTBotWebBrowser.getPaletteViewer();
		try {
			...
			public Menu run() {
				...
				try {
					Menu[] menus = ReflectionsHelper.getPrivateFieldValue(
							RunningPlatform.isOSX() ? Display.class : Decorations.class, "menus", menusHolder,
							Menu[].class);
					if (menus != null) {
	 ...
	 * @return
	 */
	public static final List<PaletteContainer> getPaletteRootContainers() {
		List<PaletteContainer> result = null;
		final PaletteViewer paletteViewer = SWTBotWebBrowser.getPaletteViewer();
		result = Display.syncExec(new ResultRunnable<LinkedList<PaletteContainer>>() {
	...
	public static final List<PaletteEntry> getPaletteContainerItems(final PaletteContainer paletteContainer) {
		...
		result = Display.syncExec(new ResultRunnable<LinkedList<PaletteEntry>>() {
	...
	/**
	 * Returns true if JBoss Tools Palette Contains Root Palette Container with
	 * label paletteContainerLabel
	 * 
	 * @param paletteContainerLabel
	 * @return
	 */
	public static boolean paletteContainsRootPaletteCotnainer(String paletteContainerLabel) {
		boolean result = false;
		List<PaletteContainer> paletteContainers = SWTBotWebBrowser.getPaletteRootContainers();
	...
	/**
	 * Returns selected text in first selection range. Works correctly only when
	...
	public String getSelectionText() {
		...
		if (selection != null && selection.getRangeCount() > 0) {
			...
			if (selectedNode.getNodeName().equals("#text")) {
				...
				result = selectedNode.getNodeValue().substring(beginIndex, endIndex);
			}
		}
	...
	private class DomMouseEvent implements nsIDOMMouseEvent {
		...
		@Override
		public void initUIEvent(String arg0, boolean arg1, boolean arg2, nsIDOMAbstractView arg3, int arg4) {
		}
		...
		@Override
		public void initMouseEvent(String arg0, boolean arg1, boolean arg2, nsIDOMAbstractView arg3, int arg4, int arg5,
				int arg6, int arg7, int arg8, boolean arg9, boolean arg10, boolean arg11, boolean arg12, int arg13,
				nsIDOMEventTarget arg14) {
		}
	}

	/**
	 * Returns node attribute attributeName value or null when node doesn't have
	 ...
	 * 
	 * @param containsWholeNodes
	 *            if true selection has to contain whole nodes
	 * @param values
	 * @return
	 */
	public boolean selectionContainsNodes(boolean containsWholeNodes, String... values) {
		boolean result = true;

		List<nsIDOMNode> selectedNodes = mozillaEditor.getXulRunnerEditor().getSelectedNodes();
	...
	public nsIDOMDocument getDomDocument() {
		...
		if (mozillaEditor != null) {
			...
			result = Display.syncExec(new ResultRunnable<nsIDOMDocument>() {
				@Override
