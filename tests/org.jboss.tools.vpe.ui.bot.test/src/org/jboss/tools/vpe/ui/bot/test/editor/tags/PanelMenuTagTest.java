...
public class PanelMenuTagTest extends AbstractTagTest {
	...
	protected void verifyTag() {
		assertVisualEditorContains(getVisualEditor(), "TD", new String[] { "class" },
				new String[] { "rich-pmenu-top-group-self-label" }, getTestPageFileName());
		assertVisualEditorContains(getVisualEditor(), "TD", new String[] { "class" },
				new String[] { "rich-pmenu-group-self-icon rich-pmenu-top-group-self-icon" }, getTestPageFileName());
		assertVisualEditorContains(getVisualEditor(), "TABLE", new String[] { "class" },
				new String[] { "dr-pmenu-top-group rich-pmenu-group" }, getTestPageFileName());
		assertVisualEditorContainsNodeWithValue(getVisualEditor(), PanelMenuTagTest.getGroupLabel(0),
				getTestPageFileName());
		assertVisualEditorContainsNodeWithValue(getVisualEditor(), PanelMenuTagTest.getGroupLabel(1),
				getTestPageFileName());
		assertVisualEditorContainsNodeWithValue(getVisualEditor(), PanelMenuTagTest.getGroupLabel(2),
				getTestPageFileName());
		assertVisualEditorNotContainNodeWithValue(getVisualEditor(), PanelMenuTagTest.getGroupItemLabel(0, 0),
				getTestPageFileName());
		// check tag selection
		...
		assertTrue("Selected text in Source Pane has to end with '" + hasToEndWith + "'" + "\nbut it is '"
				+ selectedText + "'", selectedText.trim().endsWith(hasToEndWith));
		...
		assertTrue("Selected text in Source Pane has to end with '" + hasToEndWith + "'" + "\nbut it is '"
				+ selectedText + "'", selectedText.trim().endsWith(hasToEndWith));
		assertVisualEditorContainsNodeWithValue(getVisualEditor(), PanelMenuTagTest.getSubGroupItemLabel(1, 0, 0),
				getTestPageFileName());
		assertVisualEditorContains(getVisualEditor(), "TD", new String[] { "class" },
				new String[] { "rich-pmenu-item-label" }, getTestPageFileName());
	}
