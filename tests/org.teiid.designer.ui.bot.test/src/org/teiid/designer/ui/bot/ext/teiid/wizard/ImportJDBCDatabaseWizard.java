...
public class ImportJDBCDatabaseWizard {
	...
	private void fillSecondPage() {
		...
		wizard.bot().tableInGroup("Table Types").select("TABLE");
	}

	private void fillThirdPage() {
		for (String item : itemList) {
			String[] itemArray = item.split("/");
			SWTBotTreeItem treeItem = wizard.bot().tree().getTreeItem(itemArray[0]);
	...
	private void fillFourthPage() {
		wizard.bot().checkBoxInGroup("Model Object Names (Tables, Procedures, Columns, etc...)", 0)
				.deselect();
