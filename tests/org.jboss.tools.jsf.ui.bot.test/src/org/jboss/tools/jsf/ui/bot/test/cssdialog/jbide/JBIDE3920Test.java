...
public class JBIDE3920Test extends JSFAutoTestCase{
	...
	public void testJBIDE3920(){
		...
		editorText = CSS_CLASS_NAME + "{" +  editorText; 
		editor.setText(editorText);
		editor.save();
		new ContextMenu("Open CSS Dialog").select();
		new DefaultTabItem("Text/Font").activate();
		new LabeledCombo("Font Style:").setSelection("italic");
		new LabeledCombo("Text Decoration:").setSelection("underline");
		new OkButton().click();
		assertEquals("cssclass{text-decoration:underline;font-style:italic@CHARSET\"UTF-8\";", JSFAutoTestCase.stripCSSText(editor.getText()));
		editor.close();
	}
}
