...
public class RESTfulTestBase extends WSTestBase {
	...
	public void assertCountOfProblemsExists(ProblemType problemType, String projectName, String description,
	...
	public void assertCountOfValidationProblemsExists(ProblemType problemType, String projectName, String description,
			String type, int count) {
		AbstractMarkerMatcher[] matchers = getProblemMatchers(description, projectName, type);
		
		if (count == 0 && !new ProblemExists(ProblemType.ALL).test()) {
			// prevent from false positive result when we do not expect errors and there is no error
			...
			// prevent from false negative result
	...
	protected void assertCountOfRESTServices(List<RESTfulWebService> restServices, int expectedCount) {
		assertTrue(restServices.size() + " RESTful services" + " was found instead of " + expectedCount,
	...
	protected void assertAllRESTServicesInExplorer(List<RESTfulWebService> restServices) {
		assertTrue("All RESTful services (GET, DELETE, POST, PUT) " + "should be present but they are not\nThere are "
				+ Arrays.toString(restServices.toArray()), allRestServicesArePresent(restServices));
	}
	...
	protected void assertExpectedPathOfService(String message, RESTfulWebService service, String expectedPath) {
		assertEquals(message + "Failure when comparing paths\n", expectedPath, service.getPath());
	}

	protected void assertExpectedPathOfService(RESTfulWebService service, String expectedPath) {
	...
	protected void copyRestfulResource(String resourceFile, Object... param) {
		...
		ResourceHelper.copyResourceToClassWithSave(stream, true, param);
	...
	protected void prepareSimpleRestService(String pathArgument, String pathParamArgument) {
		prepareRestService(getWsProjectName(), SIMPLE_REST_WS_RESOURCE, pathArgument, pathParamArgument);
	}

	protected void prepareRestService(String projectName, String resourceFileName, Object... param) {
	...
	protected void replaceInRestService(String projectName, String regex, String replacement) {
		openJavaFile(projectName, "org.rest.test", "RestService.java");
	...
	protected TextEditor setCursorPositionToTextInTextEditor(String text) {
		ExtendedTextEditor editor = new ExtendedTextEditor();
		int line = editor.getLineNum(StringContains.containsString(text));
		editor.setCursorPosition(line, editor.getTextAtLine(line).indexOf(text) + text.length());
		return editor;
	}
